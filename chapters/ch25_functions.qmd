...
不难发现，此函数似乎是想寻找本应在函数定义中充当变量的`group_var`。dplyr 默认直接捕获函数参数中写死的变量名（如 `group_var`），而不是评估新传入的参数名（如 `cut` 和 `carat`）。这就是“间接引用”。它产生的原因是 dplyr 采取“整洁求值”（tidy evaluation）的规则，本意是方便我们在数据框中直接引用变量名而无需特别处理，但在封装成函数时却成了绊脚石。
